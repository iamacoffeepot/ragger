...
41. `scan_blob_adjacencies.py` — Rasters the blob grid to find every walkably-adjacent cross-blob tile pair (ignoring Voronoi ridges). Records one representative pair per unique `(blob_a, blob_b)` combination in `blob_adjacencies`, patching narrow passages like city gates that the ridge sampling misses.
