...
public class ActorsApi {
    ...
    public ActorsApi(final String parentName, final ActorManager manager) {
     ...
     */
    private int run(final Lua lua) {
        final String childName = lua.toString(2);
        final String source = lua.toString(3);
        final String fullName = manager.childName(parentName, childName);
        final String result = manager.load(fullName, source);
        lua.push(result);
        return 1;
    }
     ...
     * actors:stop("child-name")
     */
    private int stop(final Lua lua) {
        final String childName = lua.toString(2);
        final String fullName = manager.childName(parentName, childName);
     ...
     */
    private int list(final Lua lua) {
        final List<String> children = manager.listChildren(parentName);
     ...
     */
    private int source(final Lua lua) {
        final String childName = lua.toString(2);
        final String fullName = manager.childName(parentName, childName);
        final String src = manager.getSource(fullName);
     ...
     */
    private int is_running(final Lua lua) {
        final String childName = lua.toString(2);
        final String fullName = manager.childName(parentName, childName);
     ...
     * actors:define("template-name", source)
     */
    private int define(final Lua lua) {
        final String templateName = lua.toString(2);
        final String source = lua.toString(3);
    ...
    private int create(final Lua lua) {
        final String childName = lua.toString(2);
        final String templateName = lua.toString(3);
        ...
        final String source = manager.getTemplate(templateName);
        if (source == null) {
        ...
        final String fullName = manager.childName(parentName, childName);
        final String result = manager.load(fullName, source, args);
        lua.push(result);
        return 1;
    }
     ...
     */
    private int templates(final Lua lua) {
        final List<String> names = manager.listTemplates();
